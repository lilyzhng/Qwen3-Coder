...
Uses transformers + PEFT + BitsAndBytes for model loading (Unsloth doesn't
support this architecture). Both runs are logged to W&B for easy comparison.
...
# Container image — uses transformers + PEFT + BNB instead of Unsloth
eval_image = (
...
# GPU config — B200 needed for 80B MoE model loading (~41GB in 4-bit)
GPU_CONFIG = "B200"
TIMEOUT_HOURS = 4
...
```"""
    ...
    def load_model_bnb(model_name: str, lora_adapter: str = None):
        """Load a model with BitsAndBytes 4-bit quantization and optional LoRA adapter.

        Uses transformers + PEFT instead of Unsloth (which doesn't support
        the Qwen3-Coder-Next MoE architecture).
        ...
        if lora_adapter:
            ...
            try:
                ...
                # Auto-detect the adapter_config.json location
                from huggingface_hub import list_repo_files
                print(f"  adapter_config.json not at root, scanning subdirectories...")
                files = list_repo_files(lora_adapter)
                adapter_dirs = [f.rsplit("/adapter_config.json", 1)[0] for f in files if f.endswith("adapter_config.json")]
                if not adapter_dirs:
                    raise ValueError(f"No adapter_config.json found in {lora_adapter}")
                subfolder = adapter_dirs[0]
                print(f"  Found adapter at: {subfolder}")
                model = PeftModel.from_pretrained(model, lora_adapter, subfolder=subfolder)

        model.eval()
        return model, tokenizer

    def generate_response(model, tokenizer, question: str, max_new_tokens: int = 4096) -> str:
        """Generate response from model. Uses raw text (no chat template) for base models."""
    ...
    # ---------------------------------------------------------------------------
    print("=" * 60)
    print("UIUX Evaluation — Qwen3-Coder-Next-Base (MoE)")
    print("=" * 60)
    ...
    # ---------------------------------------------------------------------------
    # Load Models (transformers + PEFT + BNB)
    # ---------------------------------------------------------------------------
    print("\n" + "=" * 60)
    ...
    base_model, base_tokenizer = load_model_bnb(config.base_model)
    print(f"Base model loaded! Memory: {torch.cuda.memory_allocated() / 1024**3:.1f} GB")

    # Load finetuned model (base + LoRA adapter)
    lora_model = None
    lora_tokenizer = None
    if not config.base_only and config.lora_model:
        print(f"\nLoading finetuned model: {config.base_model} + {config.lora_model}")
        lora_model, lora_tokenizer = load_model_bnb(config.base_model, lora_adapter=config.lora_model)
        print(f"Finetuned model loaded! Memory: {torch.cuda.memory_allocated() / 1024**3:.1f} GB")
    ...
    for i, sample in enumerate(samples):
        ...
        try:
            base_raw = generate_response(base_model, base_tokenizer, question, max_new_tokens=config.max_new_tokens)
        except Exception as e:
        ...
        if has_lora:
            ...
            try:
                lora_raw = generate_response(lora_model, lora_tokenizer, question, max_new_tokens=config.max_new_tokens)
            except Exception as e:
...
def main(
    ...
    if not download_only:
        ...
        print("Starting UIUX evaluation on Modal...")
        print(f"  Base model: {config.base_model}")
